...
const selectPost = (post) => {
  selectedPost.value =
    selectedPost.value && selectedPost.value.no === post.no ? null : post;
};

const deletePost = (id) => {
  if (!confirm('정말로 삭제하시겠습니까?')) return;

  const idx = posts.value.findIndex((p) => p.no === id);
  posts.value.splice(idx, 1);
  selectedPost.value = null;
};

const updatePost = (updated) => {
  const idx = posts.value.findIndex((p) => p.no === updated.no);
  posts.value[idx] = updated;
};
</script>

<template>
  <h1>📌 게시판V3(Component)</h1>

  <PostInput @add="addPost" />
